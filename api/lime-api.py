...
async def upload_model(weights: UploadFile, arch: UploadFile):
    ...
    app.model = tf.keras.models.model_from_json(json_string=model_arch)
...
async def upload_images(images: List[UploadFile]):
    ...
    for i in range(len(images)):
        ...
        image = tf.io.decode_jpeg(contents=image).numpy()
...
@app.post("/explain/")
def explain(explanation_params: Explanation, segmenter_params: Segmenter):

    # Generating explanations with the passed arguments
    generate_explanations(
        segmenter_params=segmenter_params, explanation_params=explanation_params
    )
    ...
    if explanation_params.labels_to_explain != "None":
        ...
        explain_images_with_respect_to_labels(explanation_params=explanation_params)
...
def generate_explanations(segmenter_params, explanation_params):
...
def explain_images_with_respect_to_labels(explanation_params):
    ...
    labels_to_explain = explanation_params.labels_to_explain.split(sep=",")
        ...
        for j in range(cols):
            ...
            if counter < len(labels_to_explain):
                ...
                temp, mask = explanation.get_image_and_mask(
                    label=labels_to_explain[counter],
                ...
                axes[i, j].imshow(
                    X=mark_boundaries(image=temp / 2 + 0.5, label_img=mask)
                )
    ...
    fig.savefig(fname="static/explanation.jpg")


# Function for explanining images with respect to stored predictions
def explain_images_with_respect_to_predictions(explanation_params):
    ...
    for i in range(len(image_indices)):
        ...
        ax[i, 0].imshow(X=app.original_images[image_indices[i]])
        ...
        for j in range(1, explanation_params.top_predictions + 1):
            ...
            temp, mask = explanation.get_image_and_mask(
                label=explanation.top_labels[j - 1],
                positive_only=explanation_params.positive_only,
                negative_only=explanation_params.negative_only,
                hide_rest=explanation_params.hide_rest,
            )
            ...
            # Showing the image with the superimposed mask
            ax[i, j].imshow(X=mark_boundaries(image=temp / 2 + 0.5, label_img=mask))
