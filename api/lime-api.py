...
def explain(explanation_params: Explanation,
    ...
    generate_explanations(segmenter_params=segmenter_params, 
    ...
    if explanation_params.labels_to_explain != "None":
        ...
        explain_images_with_respect_to_predictions(explanation_params=explanation_params)                           
    ...
    for i in range(rows):
        for j in range(cols):        
            ...
            if counter < len(labels_to_explain):
                ...
                axes[i, j].imshow(X=mark_boundaries(image=temp / 2 + 0.5, label_img=mask))
